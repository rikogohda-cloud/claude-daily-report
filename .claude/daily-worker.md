...
| 項目 | 値 |
|------|-----|
| ユーザー名 | __USER_DISPLAY_NAME__ |
| Slack User ID | __SLACK_USER_ID__ |
| Slack Handle | @__SLACK_HANDLE__ |
| Slack Crawler DB | __SLACK_CRAWLER_DB_PATH__ |
| 除外チャンネル（times系） | __TIMES_CHANNEL_NAME__ |
| 議事録 Notion DB ID | __MEETING_NOTES_DB_ID__ |
| 振り返りトーン | strict |
...
```
conversations_search_messages: filter_users_from=@__SLACK_HANDLE__, filter_date_during=Today, limit=50
```
→ チャンネル投稿 + DM送信の両方を含む。DM分は詳細ログの「DM」セクションに振り分ける。

### 1-D. Slack: メンション（直近3日・未返信チェック兼用）
```
conversations_search_messages: search_query=@__SLACK_HANDLE__, filter_date_after=<3日前の日付>, limit=50
```
...
→ **議事録DB（ID: `__MEETING_NOTES_DB_ID__`）配下のページは除外**（1-Lで取得するため）。議事録DB未設定の場合は除外なし。
...
### 1-L. Notion: 当日の議事録
議事録DB（ID: `__MEETING_NOTES_DB_ID__`）を `notion-query-data-sources` でクエリし、当日作成された行を取得する。
```
notion-query-data-sources: dataSourceId="__MEETING_NOTES_DB_ID__"
```
→ 当日分のページをフィルタして抽出。
...
**議事録DB未設定（`__MEETING_NOTES_DB_ID__` が空）の場合はこのステップをスキップ。**

---

## Step 2: スレッド文脈取得

Step 1で見つかったスレッドのうち、substantiveなもの**上位10件**について `conversations_replies` で全文脈を取得する。
